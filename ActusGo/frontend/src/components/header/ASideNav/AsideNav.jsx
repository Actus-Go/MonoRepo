      ...
      <div className="text-white w-full flex shadow-2xl flex-col justify-between items-center gap-6">
        ...
        <Link
          ...
          className={`profile_link ${
            isCollapsed ? "h-0 opacity-0" : "h-28"
          } transition-all  ${
            page === "profile"
              ? "active_link bg-[#efff55] text-gray-900 w-[48px] rounded-full font-bold"
              : ""
          }`}
        >
          <div className="flex flex-col items-center">
            ...
            />
            <span
              className={`uppercase grid place-items-center w-full h-full ${
                page === "profile"
                  ? "active_link bg-[#efff55] text-gray-900 font-bold"
                  : ""
              }`}
            >
      ...
      <div className="w-full flex flex-col justify-between h-full z-10  overflow-y-auto">
        ...
        <div className="flex-grow px-3 w-full">
          ...
          <NavLink
            to="/"
            className={({ isActive }) =>
              `flex items-center rounded-[34px] py-3 px-4 ${
                isActive ? "bg-[#5d5b5b]" : ""
              }`
            }
            onClick={() => getAllPosts()}
          ...
          >
            ...
            {!isCollapsed && (
              <>
                ...
                <div className="ml-auto bg-[#eaf953] text-black text-xs rounded-full px-2 py-1">
                  999+
                </div>
              </>
            )}
          ...
          <div className="relative  cursor-not-allowed  flex items-center">
            ...
            <NavLink
              to="/analysis"
              className={({ isActive }) =>
                `flex items-center pointer-events-none rounded-[34px] py-3 px-4 ${
                  isActive ? "bg-[#5d5b5b]" : " "
                }`
              }
            ...
            >
              <BarChart2 size={24} />
              {!isCollapsed && (
                <span className="ml-4 text-white">Analysis</span>
              )}
            </NavLink>
          ...
          <div
            className={`mt-[40px] px-4 py-2 w-full transition-all ${
              isCollapsed && "hidden"
            }`}
          >
                  ...
                  <div className="relative">
                    <div
                      className={`w-10 h-6 rounded-full shadow-inner ${
                        isPlayMode ? "bg-[#7047eb]" : "bg-gray-400"
                      }`}
                    ></div>
                    <div
                      className={`dot absolute w-4 h-4 bg-white rounded-full shadow left-1 top-1 transition ${
                        isPlayMode ? "translate-x-4" : ""
                      }`}
                    ></div>
                  </div>
                )}
            ...
            <div className="relative">
                ...
                >
                  ...
                  {!isCollapsed && (
                    <div className="relative">
                      <div
                        className={`w-10 h-6 rounded-full shadow-inner ${
                          isDarkMode ? "bg-[#7047eb]" : "bg-gray-400"
                        }`}
                      ></div>
                      <div
                        className={`dot absolute w-4 h-4 bg-white rounded-full shadow left-1 top-1 transition ${
              ...
              <div
                className={`absolute inset-0 ${
                  isCollapsed ? "text-sm" : ""
                }  text-sm bg-opacity-80 text-white text-center flex items-center justify-center rounded transition-opacity duration-300 opacity-100`}
              >
                Coming Soon
              </div>
            </div>
            ...
            <div className="relative">
              <Link
                to="/help"
                className="flex opacity-10 items-center rounded-[34px] px-4 text-white py-2 "
              >
                <HelpCircle size={28} />
                {!isCollapsed && (
                  <span className="ml-4 text-white">Help Center</span>
                )}
              </Link>
              <div
                className={`absolute inset-0 ${
            ...
            <Link
              to="/logout"
              className="flex items-center px-4 text-red-500 py-2 "
            >
              <LogOut size={28} />
              {!isCollapsed && (
