...
router.post('/register', async (req, res) => {
  try {
    ...
    // hitwebhook for register the user in the both systems
    const result = await hitWebHook(registeredUser);
    if (!result) {
      await User.findByIdAndRemove(registeredUser.id);
      throw Error('An error has been occured please try again.');
    }
    ...
    res.status(200).json({
      ...
      user: {
        ...
        lastLocation: registeredUser.lastLocation
      }
    });
  } catch (error) {
    console.log(error);
...
const hitWebHook = async user => {
  const url = process.env.ACTUS_GO_URL;
  const body = {
    user
  };
  const headers = {
