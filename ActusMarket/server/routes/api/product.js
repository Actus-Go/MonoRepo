...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const productCoupon = req.body.coupon;

      if (!sku && !productCoupon) {
        return res.status(400).json({ error: 'You must enter sku & coupon.' });
      }
  ...
  async (req, res) => {
    try {
      ...
      const clonedProducts = products.map(product => {
        ...
        product.priceAfterCoupon = priceAfterCoupon;
        return product;
      });
